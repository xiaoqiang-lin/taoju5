 ...
 * @LastEditTime: 2021-08-05 15:06:00
 */
...
Future openRollingCurtainProductAttributeDialog(BuildContext context,
    ...
    Function()? addToCart,
    Function()? buy}) {
  if (!Get.isRegistered<RollingCurtainProductAttributeController>(
      tag: "${product.id}")) {
    Get.lazyPut(
        () => RollingCurtainProductAttributeController(product: product),
        tag: "${product.id}");
  } else {
    /// 如果 [CurtainProductAttributeController] 注册过，则将p重新赋值
    product.attribute =
        Get.find<RollingCurtainProductAttributeController>(tag: "${product.id}")
            .product
            .attribute;
  }
  return showDialog(
...
class RollingCurtainProductAttributeDialog extends StatelessWidget {
  ...
  const RollingCurtainProductAttributeDialog(
      {Key? key, required this.product, this.footerBuilder})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return GetBuilder<RollingCurtainProductAttributeController>(
      ...
      tag: "${product.id}",
      builder: (_) {
        return FutureLoadStateBuilder<RollingCurtainProductAttributeController>(
            controller: _,
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              Text(
                                  "${product.attribute.measureData.room.label}",
                                  style: TextStyle(
                                      fontSize: R.dimen.sp12,
                                      color: R.color.ff333333)),
                              Text(
                                  "${product.attribute.measureData.room.value}",
                                  style: TextStyle(
                                      fontSize: R.dimen.sp12,
                                      color: R.color.ff333333))
                            ],
                                ...
                                Expanded(
                                  child: SizedTextField(
                                    ...
                                    initialValue: product
                                        .attribute.measureData.size.width
                                        ?.toStringAsFixed(2),
                                    onChanged: _.setWidth,
                                    shakeController: product.attribute
                                        .measureData.size.heightShakeController,
                                    hintColor: product.attribute.measureData
                                            .size.heightError
                                        ? R.color.ffff5005
                                        : R.color.ffbcbcbc,
                                    decoration:
                                        InputDecoration(hintText: "请输入宽"),
                                  ),
                                ),
                              ],
                            ),
                            ...
                            children: [
                              ...
                              Expanded(
                                child: SizedTextField(
                                  ...
                                  shakeController: product.attribute.measureData
                                      .size.heightShakeController,
                                  hintColor: product.attribute.measureData.size
                                          .heightError
                                      ? R.color.ffff5005
                                      : R.color.ffbcbcbc,
